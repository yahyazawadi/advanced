...
public class HelloController {
    ...
    public void saveRealEstateChanges() throws IOException {
        ...
        if (imageBytes != null && imageBytes.length > 0) {
            ...
            Image image = new Image(inputStream);
            imageView.setImage(image); // تحديث العرض في واجهة المستخدم
        } else {
    ...
    public void uploadImage() {
        ...
        if (selectedFile != null) {
            ...
            if (imageBytes != null) {
                ByteArrayInputStream inputStream = new ByteArrayInputStream(imageBytes);
                Image image = new Image(inputStream);
                imageView.setImage(image); // تحديث العرض
    ...
    public void deleteImage() {
        RealEstate e = realEstateDOAImp.getRealEstateById(18);
        e.setImag(null); // إزالة الصورة من الكائن
        realEstateDOAImp.update(e); // تحديث الكائن في قاعدة البيانات
        imageView.setImage(null); // إزالة الصورة من واجهة المستخدم
        imageBytes = null; // إعادة تعيين المتغير
